...
export default async function handler(req, res) {
  ...
  if (containsBadLanguage(syllabusText) || containsBadLanguage(subject)) {
    return res.status(400).json({ error: "Inappropriate content detected in syllabus/subject." });
  }
...
3) 10 Numericals with fully worked solutions (where subject fits; else 10 practice Q&A).
  ...
  try {
    if (!client) {
      return res.json({
        ...
        notes: `Sample notes for ${subject}. (Add your OPENAI_API_KEY to generate full content.)`,
